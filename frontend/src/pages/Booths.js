...
export const Booths = () => {
  ...
  const loadData = async () => {
    try {
      const [boothsRes, bookingsRes] = await Promise.all([
        ...
        apiClient.get('/bookings?resource_type=booth')
      ]);
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      await apiClient.post('/bookings', {
        ...bookingForm,
        resource_type: 'booth',
        resource_id: selectedBooth.id,
        resource_name: selectedBooth.name
      });
  ...
  const getBoothBookings = (boothId) => {
    return bookings.filter(b => b.resource_id === boothId);
  };

  const calendarEvents = useMemo(() => {
    if (!selectedBooth) return [];
    
    return getBoothBookings(selectedBooth.id).map(booking => ({
      id: booking.id,
  ...
  return (
    <div className="space-y-6" data-testid="booths-page">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {booths.map((booth) => (
          <Card key={booth.id} className="bg-white border-gray-200 rounded-sm">
            <CardHeader className="border-b border-gray-200">
              <CardTitle className="font-secondary uppercase text-black">{booth.name}</CardTitle>
            </CardHeader>
            <CardContent className="pt-4">
              <p className="text-gray-600 font-primary text-sm mb-4">{booth.description}</p>
              
              <div className="space-y-2 mb-4">
                <div className="text-sm font-secondary uppercase text-black">Próximas reservas:</div>
                {getBoothBookings(booth.id).slice(0, 3).map((booking) => (
                ...
                {getBoothBookings(booth.id).length === 0 && (
              ...
              <div className="flex gap-2">
                <Button
                  data-testid={`view-calendar-${booth.name}`}
                  onClick={() => {
                ...
                <Button
                  data-testid={`add-booking-${booth.name}`}
                  onClick={() => {
                    setSelectedBooth(booth);
